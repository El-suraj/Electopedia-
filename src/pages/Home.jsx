  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <h3 className={`font-body font-semibold text-base leading-tight mb-1
          ${accent ? 'text-white' : 'text-ink-900'}`}>
          {title}
        </h3>
        <p className={`text-sm leading-relaxed ${accent ? 'text-forest-200' : 'text-ink-500'}`}>
          {description}
        </p>
      </div>
...
export default function Home() {
  ...
  const dateStr = electionDate.toLocaleDateString(lang === 'ha' ? 'ha-NG' : 'en-NG', {
    ...
    <div className="min-h-screen bg-surface page-root">
      ...
      <section className="px-4 pt-6 pb-8 anim-6">
        ...
        <div className="flex flex-wrap gap-2">
          <TopicPill icon="🗳️" label={lang === 'ha' ? 'Wane zai zaɓe?' : 'Who can vote?'}          to="/topic/who_can_vote"         color="green" />
          <TopicPill icon="🔒" label={lang === 'ha' ? 'Sirrin zaɓe'   : 'Secret ballot'}            to="/topic/secrecy_of_vote"       color="green" />
          <TopicPill icon="🪪" label={lang === 'ha' ? 'Karɓar PVC'    : 'Get your PVC'}             to="/topic/get_pvc"               color="gold"  />
          <TopicPill icon="🛡️" label={lang === 'ha' ? 'Kariya'        : 'Against intimidation'}     to="/topic/protection_from_intimidation" color="slate" />
          <TopicPill icon="📊" label={lang === 'ha' ? 'Sakamakon'     : 'After voting'}             to="/topic/after_voting"          color="slate" />
          <TopicPill icon="🏛️" label={lang === 'ha' ? 'Tsayawa takara': 'Run for office'}           to="/topic/can_i_run"             color="gold"  />
        </div>
      </section>
      ...
      <section className="px-4 pb-8">
        <div className="flex items-start gap-3 bg-gold-50 border border-gold-100 rounded-2xl px-4 py-3.5">
          ...
          <p className="text-ink-600 text-xs font-body leading-relaxed">
            ...
            <a href="https://www.inec.gov.ng" target="_blank" rel="noopener noreferrer"
              className="text-forest-700 font-semibold hover:underline">
              inec.gov.ng
            </a>
            {lang === 'ha' ? '. Kwanakin da aka nuna na nuni ne.' : '. Dates shown are indicative.'}
          </p>
        </div>
      </section>
    </div>
